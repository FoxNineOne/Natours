...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    return next(new AppError("Incorrect email or password supplied", 401));
  }
...
// Only for rendered pages, will not  error
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // 2) If success, check if user still exists
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError("There is no user with supplied email address", 404),
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  // 3) Update changedPasswordAt property for curr user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  //findByIDandUpdate would not work as pre.save and validation would not be considered!
